...
## `findall` - найти все вхождения, возвращает список
...
## `finditer` - находит все вхождения, возвращает итератор
...
## `search` - находит первое вхождение
...
|`re.M`|`re.MULTILINE`|используется совместно с метасимволами `^` и `$`, в первом случае возвращает совпадения в начале всего текста или в начале каждой новой строки (сразу после `\n`), во втором — в конце всего текста или в конце каждой строки (сразу перед `\n`)|
