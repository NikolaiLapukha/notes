...
Массивы `array` являются типами и ведут себя очень похоже на списки, за исключением того, что тип хранящихся в них объектов ограничен. Тип указывается во время создания объекта с помощью кода типа, который представляет собой один символ.
...
Модуль `array` предоставляет несколько полезных методов для работы с массивами:

- `append(x)`: добавляет элемент `x` в конец массива.
- `extend(iterable)`: добавляет элементы из итерируемого объекта в конец массива.
- `insert(i, x)`: вставляет элемент `x` в позицию `i`.
- `remove(x)`: удаляет первое вхождение элемента `x`.
- `pop([i])`: удаляет и возвращает элемент в позиции `i` (по умолчанию последний элемент).
- `index(x)`: возвращает индекс первого вхождения элемента `x`.
- `reverse()`: разворачивает массив.
- `buffer_info()`: возвращает кортеж (адрес памяти, длина массива).
...
- `tobytes()`: преобразует массив в байтовую строку.
- `frombytes(bytes)`: добавляет элементы из байтовой строки.

# Тип данных deque

Тип данных `deque` представляет собой коллекцию, которая похожа на список, за исключением того, что добавлять и удалять элементы можно либо в начало (слева), либо в конец (справа). Тип данных `deque` можно использовать как очередь или стек.

 Он имеет следующие операции:

- emptyempty — проверка на наличие элементов,
- pushBackpushBack (запись в конец) — операция вставки нового элемента в конец,
- popBackpopBack (снятие с конца) — операция удаления конечного элемента,
- pushFrontpushFront (запись в начало) — операция вставки нового элемента в начало,
- popFrontpopFront (снятие с начала) — операция удаления начального элемента.
...
`extendleft(iterable)`:  Добавляет элементы из iterable в начало deque (порядок элементов будет обратным).  
`rotate(n)`:  Поворачивает элементы deque на n позиций. Если n положительное, элементы перемещаются вправо, если отрицательное — влево

# тип данных heapq

Модуль `heapq` в Python предоставляет функции для работы с кучами (heaps), которые являются разновидностью приоритетных очередей. Куча — это бинарное дерево, в котором каждый родительский узел имеет значение меньшее или равное значению любого из его дочерних узлов. В Python реализована минимальная куча, где наименьший элемент всегда находится в корне (индекс 0).
...
- **Удаление и возврат наименьшего элемента**
...
- **Одновременное добавление и удаление элемента**
...
- **Замена наименьшего элемента и возврат старого**
...
- **Нахождение n наименьших или наибольших элементов**

```python
data = [21, 1, 45, 78, 3, 5]
smallest_three = heapq.nsmallest(3, data)
largest_three = heapq.nlargest(3, data)
print(smallest_three)  # [1, 3, 5]
print(largest_three)  # [78, 45, 21]
```
